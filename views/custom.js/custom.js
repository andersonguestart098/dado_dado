...
async function salvar_registro(id,a) {
    identificareditar=true

    // Recuperar os valore dos campos
    var datahora_valor = document.getElementById("datahora_text" + id).innerText;
    var vendedor_valor = document.getElementById("valor_vendedor" + id).innerText;
    var nropedido_valor = document.getElementById("valor_nropedido" + id).innerText;
    var cliente_valor = document.getElementById("valor_cliente" + id).innerText;
    var tipodefaturamento_valor = document.getElementById("valor_tipodefaturamento" + id).innerText;
    var valordopedido_valor = document.getElementById("valor_valordopedido" + id).innerText;
    var formapgto_valor = document.getElementById("valor_formapgto" + id).innerText;
    var retiraentrega_valor = document.getElementById("valor_retiraentrega" + id).innerText;
    var localdaentrega_valor = document.getElementById("valor_localdaentrega" + id).innerText;
    var localdecobranca_valor = document.getElementById("valor_localdecobranca" + id).innerText;
    var obs1_valor = document.getElementById("valor_obs1" + id).innerText;
    var fretedestacado_valor = document.getElementById("valor_fretedestacado" + id).innerText;
    var valorfrete_valor = document.getElementById("valor_valorfrete" + id).innerText;
    var dataentrega_valor = document.getElementById("valor_dataentrega" + id).innerText;


    if(!limitador[0]){
        var operadornf_valor = document.getElementById("operador" + id).value;
    }else{
        var operadornf_valor = document.getElementById("Pvalor_operador" + id).innerText;
    }
    if(!limitador[1]){
        var statusnf_valor = document.getElementById("coisa" + id).value;
    }else{
        var statusnf_valor = document.getElementById("Pvalor_coisa" + id).innerText;
    }
    if(!limitador[2]){
        var obs2_valor = document.getElementById("obs2_text" + id).value;
    }else{
        var obs2_valor = document.getElementById("Pvalor_obs2" + id).innerText;
    }
    if(!limitador[3]){
        var numeronf_valor = document.getElementById("numeronf_text" + id).value;
    }else{
        var numeronf_valor = document.getElementById("Pvalor_numeronf" + id).innerText;
    }
    if(!limitador[4]){
        var exped_valor = document.getElementById("exped" + id).value;
    }else{
        var exped_valor = document.getElementById("Pvalor_exped" + id).innerText;
    }
    if(!limitador[5]){
        var quemrecebeu_valor = document.getElementById("quem" + id).value;
    }else{
        var quemrecebeu_valor = document.getElementById("Pvalor_quemrecebeu" + id).innerText;
    }
                 ...
                 "&formapgto=" +
                  formapgto_valor +
                   "&retiraentrega=" +
                    retiraentrega_valor +
                     "&localdaentrega=" +
                      localdaentrega_valor +
                       "&localdecobranca=" + 
                       localdecobranca_valor +
                        "&obs1=" + obs1_valor +
                         "&fretedestacado=" +
                          fretedestacado_valor +
                           "&valorfrete=" +
                            valorfrete_valor +
                             "&dataentrega=" +
                              dataentrega_valor +
...
/*let dadosForm = `id=${id}&datahora=${datahora_valor}&vendedor=${vendedor_valor}&nropedido=`+
`${nropedido_valor}&cliente=${cliente_valor}&tipodefaturamento=${tipodefaturamento_valor}`+
`&valordopedido=${valordopedido_valor}&formapgto=${formapgto_valor}&retiraentrega=`+
`${retiraentrega_valor}&localdaentrega=${localdaentrega_valor}&localdecobranca=`+ 
`${localdecobranca_valor}&obs1=${obs1_valor}&fretedestacado=${fretedestacado_valor}`+
`&valorfrete=${valorfrete_valor}`+
"&operadornf=" +
 operadornf_valor  +
  "&statusnf=" +
   statusnf_valor +
